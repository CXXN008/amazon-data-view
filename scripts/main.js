document.addEventListener('DOMContentLoaded', function () {
  ...
  let topField = demo.querySelector('.top-field');
  ...
  function filter() {
    ...
    grid.filter(function (item) {
      ...
      console.log(
        ...
        'isAchoiceMatch: ', isAchoiceMatch,
        'isAplusMatch: ', isAplusMatch,
        'isVdieoMatch: ', isVdieoMatch,
        'isImageMatch: ', isImageMatch,
        'isPvideoMatch: ', isPvideoMatch,
  ...
  function formatElements(item, i) {
    ...
    ['best', 'achoice', 'aplus', 'video', 'image', 'pvideo'].forEach(e => {
      let hStr = `
        <div class="pretty p-svg p-curve p-locked p-check">
              <input class="card-`+ e + `" type="checkbox" ` + (item[e] > 0 ? 'checked' : '') + ` />
    ...
    let itemTemplate = '' +
      ...
      '<div class="card-asin">' + item.asin + '</div>' +
      '</div>' +
      '<hr/>' +
      '<img class="card-img" src="' + 'https://images-na.ssl-images-amazon.com/images' + item.imglink + '"></img>' +
      '<hr/>' +
      '<div class="card-title">' + item.name + '</div>' +
      '<div class="card-extra">' +
      '<div class="card-rating">' +
      '<span>评分:' + item.star + '</span>' +
      '<p class="starability-result" data-rating="' + Math.floor(item.star) + '"></p>' +
      '</div>' +
      ...
      '</div>' +
      '<div class="card-extra">' +
      '<div class="card-bot card-date">上市: ' + getLocalTime(item.date_first_on).split(' ')[0] + '</div>' +
      '<div class="card-bot card-comment">评价: ' + item.ask + '</div>' +
      '<div class="card-bot card-ask">问答: ' + item.reviews + '</div>' +
      '</div>' +
    ...
    // https://images-na.ssl-images-amazon.com/images
    itemElem.firstChild.style.backgroundImage = 'https://images-na.ssl-images-amazon.com/images' + item.imglink
